...
export const CreateQuizPage = () => {
  ...
  const handleSave = async () => {
    ...
    const formattedQuestions: Question[] = questions.map((q, idx) => ({
      id: q.id || `q_${idx + 1}_${Math.random().toString(36).substr(2, 5)}`,
      text: q.text || `Question ${idx + 1}`,
      options: (q.options && q.options.length === 4)
        ? q.options.map((opt, i) => opt || `Option ${i + 1}`)
    ...
    const quiz: Quiz = {
      id: currentQuiz?.id || Math.random().toString(36).substr(2, 9),
      title: title || 'Custom Battle Arena',
    ...
    socket.emit('join_room', { roomCode: activeRoomCode, player: hostPlayer, quiz: savedQuiz });
